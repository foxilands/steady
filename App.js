...
function quincenaLabel(q) {
  ...
  const monthName = new Date(q.year, q.month, 1).toLocaleDateString(undefined, {
    month: "short",
  });
...
function quincenaDays(q) {
  ...
  for (let day = start; day <= end; day++) {
    days.push({
      key: dateKey(new Date(q.year, q.month, day)),
      day,
    });
  }
...
function formatCanEquivalent(value) {
  if (!Number.isFinite(value) || value <= 0) {
    return "0 cans";
  }

  return `${value < 10 ? value.toFixed(2) : value.toFixed(1)} cans`;
}

function DayChip({ day, count, logged, limit, isToday, selected, onClick, chipStyle }) {
...
function ActiveTimerCard({ timerStart, timerLimit, onStop }) {
  ...
  return (
    ...
    >
      <View style={styles.rowBetween}>
        ...
        <TouchableOpacity onPress={onStop} style={styles.btnSecondary}>
          <Square size={12} color={C.textDim} />
...
export default function App() {
  ...
  const [canPriceInput, setCanPriceInput] = useState("85");

  const [canSizeInput, setCanSizeInput] = useState("15");
  ...
  const [currentDayKey, setCurrentDayKey] = useState(() => dateKey());
    ...
    async function initNotifications() {
      try {
        if (Platform.OS === "android") {
          ...
          await Notifications.setNotificationChannelAsync("timer-progress", {
            name: "Active Pouch Timer",
        ...
        if (existingStatus !== "granted" && isMounted) {
  ...
  useEffect(() => {
    ...
    const scheduleMidnightUpdate = () => {
      ...
      const delay = Math.max(1000, nextDay.getTime() - currentDate.getTime() + 250);
    ...
    async function loadData() {
      try {
        ...
        if (savedLimit && !isNaN(Number(savedLimit))) {
          setLimit(Math.max(0, Number(savedLimit)));
        }

        const savedPrice = await AsyncStorage.getItem("pouch-can-price");

        if (savedPrice && !isNaN(Number(savedPrice))) {
          const parsedPrice = Math.max(0, Number(savedPrice));

          setCanPrice(parsedPrice);
          setCanPriceInput(String(parsedPrice));
        }

        const savedSize = await AsyncStorage.getItem("pouch-can-size");

        if (savedSize && !isNaN(Number(savedSize))) {
          const parsedSize = Math.max(1, Number(savedSize));

          setCanSize(parsedSize);
          setCanSizeInput(String(parsedSize));
        }
        ...
        if (savedTimerStart && !isNaN(Number(savedTimerStart))) {
          setTimerStart(Number(savedTimerStart));
        }
      } catch (e) {
      } finally {
        setLoaded(true);
  ...
  useEffect(() => {
    ...
    AsyncStorage.setItem("pouch-log", JSON.stringify(log)).catch(() => {});
  }, [log, loaded]);
  ...
  const saveCanPrice = async (rawVal) => {
    const safeValue = parseFloatValue(rawVal, canPrice, 0, 5000);

    setCanPrice(safeValue);
    setCanPriceInput(String(safeValue));
  ...
  const saveCanSize = async (rawVal) => {
    const safeValue = parseIntValue(rawVal, canSize, 1, 100);

    setCanSize(safeValue);
    setCanSizeInput(String(safeValue));
  ...
  const adjustDay = (dayKey, delta) => {
    setLog((currentLog) => {
      const current = currentLog[dayKey] || 0;
    ...
    try {
      ...
      if (secondsRemaining > 0) {
        await Notifications.scheduleNotificationAsync({
          content: {
            title: "Time is up! ⏱️",
            body: `It has been ${durationMinutes} minutes. Time to take your pouch out.`,
            sound: true,
  ...
  useEffect(() => {
    if (!loaded || !notificationsReady || timerStart === null) {
      return;
    }

    scheduleTimerNotifications(timerStart, timerLimit);
  }, [loaded, notificationsReady, timerStart, timerLimit]);
  ...
  const selectCalMode = (mode) => {
    setSelectedDay(null);
    setCalMode(mode);
  };

  const setSyncedQuincena = (nextValue) => {
    const next = typeof nextValue === "function" ? nextValue(quincena) : nextValue;
  ...
  const shiftMonth = (direction) => {
    const date = new Date(calMonth.year, calMonth.month + direction, 1);
  ...
  const summarizeCostPeriod = (periodDays) => {
    const pouchCount = periodDays.reduce((sum, day) => sum + (log[day.key] || 0), 0);

    const loggedDayCount = periodDays.filter((day) => log[day.key] !== undefined).length;

    const calendarDayCount = periodDays.filter((day) => day.key <= key).length;

    const cost = costFor(pouchCount);

    return {
      pouchCount,
      cost,
      cans: canSize > 0 ? pouchCount / canSize : 0,
      loggedDayCount,
      calendarDayCount,
      avgCostPerLoggedDay: loggedDayCount > 0 ? cost / loggedDayCount : 0,
      avgCostPerCalendarDay: calendarDayCount > 0 ? cost / calendarDayCount : 0,
    };
  };

  const costDayKey = selectedDay || key;

  const costDayCount = log[costDayKey] || 0;

  const dayCostSummary = {
    pouchCount: costDayCount,
    cost: costFor(costDayCount),
    cans: canSize > 0 ? costDayCount / canSize : 0,
  };

  const quincenaCostSummary = summarizeCostPeriod(qDays);

  const monthCostSummary = summarizeCostPeriod(mDays);

  const costPerPouch = canSize > 0 ? canPrice / canSize : 0;
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      <ScrollView contentContainerStyle={styles.container}>
        ...
        {timerStart === null ? (
          ...
          <ActiveTimerCard timerStart={timerStart} timerLimit={timerLimit} onStop={stopTimer} />
        )}
        ...
        <View
          style={[
            styles.card,
            overLimit ? styles.cardOver : atLimit ? styles.cardAtLimit : styles.cardWhite,
          ]}
        >
          <View style={styles.rowBetween}>
            <TouchableOpacity onPress={() => adjust(-1)} style={styles.btnRoundSecondary}>
              <Minus size={20} color={C.textDim} />
            </TouchableOpacity>
          ...
          {overLimit && (
            <Text style={styles.warningText}>
              Over your limit today — no judgment, just noting it.
            </Text>
          )}

          {atLimit && !overLimit && (
            <View style={styles.rowCenterGap}>
              <Check size={12} color={C.textDim} />
        ...
        {trendUp && (
          <View style={styles.trendBox}>
            ...
            <Text style={styles.trendText}>
              Your daily count has been trending up this week. Worth noticing before it becomes the
              new normal.
            </Text>
          </View>
        )}
        ...
        >
          <TouchableOpacity
            accessibilityLabel={showCalendar ? "Collapse calendar" : "Expand calendar"}
            onPress={() => setShowCalendar(!showCalendar)}
            style={styles.accordionHeader}
          >
            <View style={styles.rowGap}>
              <CalendarIcon size={16} color={C.textDim} />

              <Text style={styles.cardTitle}>Calendar</Text>
            </View>

            {showCalendar ? (
              <ChevronUp size={18} color={C.textDim} />
            ) : (
            ...
            <View style={styles.accordionBody}>
              <View style={styles.rowBetween}>
                <View style={styles.segmentedControl}>
                  <TouchableOpacity
                    onPress={() => selectCalMode("quincena")}
                    style={[styles.segmentBtn, calMode === "quincena" && styles.segmentBtnActive]}
                  >
                    <Text
                      style={[
                        styles.segmentText,
                        calMode === "quincena" && styles.segmentTextActive,
                      ]}
                    >
                      Quincena
                    </Text>
                  ...
                  <TouchableOpacity
                    onPress={() => selectCalMode("month")}
                    style={[styles.segmentBtn, calMode === "month" && styles.segmentBtnActive]}
                  >
                    <Text
                      style={[styles.segmentText, calMode === "month" && styles.segmentTextActive]}
                    >
                      Month
                    </Text>
                  </TouchableOpacity>
                ...
                {calMode === "quincena" ? (
                  <View style={styles.rowGap}>
                    <TouchableOpacity onPress={() => setSyncedQuincena(prevQuincena)}>
                      <ChevronLeft size={16} color={C.textDim} />
                    </TouchableOpacity>

                    <Text style={styles.calNavTitle}>{quincenaLabel(quincena)}</Text>

                    <TouchableOpacity onPress={() => setSyncedQuincena(nextQuincena)}>
                  ...
                  <View style={styles.rowGap}>
                    ...
                    <TouchableOpacity accessibilityLabel="Next month" onPress={() => shiftMonth(1)}>
                      <ChevronRight size={16} color={C.textDim} />
                    </TouchableOpacity>
                  </View>
                )}
              ...
              {calMode === "quincena" ? (
                ...
                <View style={styles.grid7}>
                  ...
                  {Array.from({
                    ...
                    <View key={`blank-${i}`} style={[styles.dayChip, styles.dayChipBlank]} />
                  ))}
              ...
              {selectedDay && (
                <View style={styles.selectedDayBar}>
                  ...
                  <View style={styles.rowGap}>
                    <TouchableOpacity
                      accessibilityLabel="Decrease selected day count"
                      onPress={() => adjustDay(selectedDay, -1)}
                    ...
                    <TouchableOpacity
                      accessibilityLabel="Increase selected day count"
                      onPress={() => adjustDay(selectedDay, 1)}
        ...
        >
          <TouchableOpacity
            accessibilityLabel={
              showCostBreakdown ? "Collapse cost breakdown" : "Expand cost breakdown"
            }
            onPress={() => setShowCostBreakdown(!showCostBreakdown)}
            style={styles.accordionHeader}
          >
            <View style={styles.rowGap}>
              <DollarSign size={16} color={C.textDim} />
          ...
          {showCostBreakdown && (
            <View style={styles.accordionBody}>
              <View style={styles.costRateRow}>
                <Text style={styles.subtextSmall}>Based on your current can settings</Text>

                <Text style={styles.subtextSmall}>{mxn.format(costPerPouch)} / pouch</Text>
              </View>

              <View style={styles.costSummaryRow}>
                <View style={styles.costSummaryMain}>
                  <Text style={styles.costPeriodLabel}>
                    {costDayKey === key ? "Today" : dateLabelFromKey(costDayKey)}
                  </Text>

                  <Text style={styles.costPeriodMeta}>
                    {dayCostSummary.pouchCount}{" "}
                    {dayCostSummary.pouchCount === 1 ? "pouch" : "pouches"} ·{" "}
                    {formatCanEquivalent(dayCostSummary.cans)}
                  </Text>
                </View>

                <Text style={styles.costAmount}>{mxn.format(dayCostSummary.cost)}</Text>
              </View>

              <View style={styles.costSummaryRow}>
                <View style={styles.costSummaryMain}>
                  <Text style={styles.costPeriodLabel}>Quincena · {quincenaLabel(quincena)}</Text>

                  <Text style={styles.costPeriodMeta}>
                    {quincenaCostSummary.pouchCount} pouches ·{" "}
                    {formatCanEquivalent(quincenaCostSummary.cans)}
                  </Text>

                  <Text style={styles.costAverageText}>
                    {quincenaCostSummary.loggedDayCount > 0
                      ? `${mxn.format(
                          quincenaCostSummary.avgCostPerLoggedDay
                        )} / logged day · ${mxn.format(
                          quincenaCostSummary.avgCostPerCalendarDay
                        )} / calendar day`
                      : quincenaCostSummary.calendarDayCount > 0
                        ? `${mxn.format(0)} / calendar day`
                        : "Period not started"}
                  </Text>
                </View>

                <Text style={styles.costAmount}>{mxn.format(quincenaCostSummary.cost)}</Text>
              </View>

              <View style={[styles.costSummaryRow, styles.costSummaryRowLast]}>
                <View style={styles.costSummaryMain}>
                  <Text style={styles.costPeriodLabel}>
                    {monthLabel(calMonth.year, calMonth.month)}
                  </Text>
                  ...
                  <Text style={styles.costPeriodMeta}>
                    {monthCostSummary.pouchCount} pouches ·{" "}
                    {formatCanEquivalent(monthCostSummary.cans)}
                  </Text>

                  <Text style={styles.costAverageText}>
                    {monthCostSummary.loggedDayCount > 0
                      ? `${mxn.format(
                          monthCostSummary.avgCostPerLoggedDay
                        )} / logged day · ${mxn.format(
                          monthCostSummary.avgCostPerCalendarDay
                        )} / calendar day`
                      : monthCostSummary.calendarDayCount > 0
                        ? `${mxn.format(0)} / calendar day`
                        : "Period not started"}
                  </Text>
                </View>

                <Text style={styles.costAmount}>{mxn.format(monthCostSummary.cost)}</Text>
              </View>

              <Text style={styles.costHint}>
                Day follows your selected calendar date; quincena and month follow the calendar
                periods above.
              </Text>
            </View>
          )}
            ...
            {!showLimitEdit ? (
              ...
              <View>
                ...
                <TouchableOpacity
                  ...
                  onPress={() => setShowLimitEdit(false)}
                  style={{
                    alignItems: "flex-end",
                    marginTop: 12,
                  }}
          ...
          <View style={[styles.card, styles.cardWhite]}>
            {!showCostEdit ? (
              <View style={styles.rowBetween}>
                ...
                <TouchableOpacity
                  accessibilityLabel="Edit can cost and size"
                  onPress={() => {
                    setCanPriceInput(String(canPrice));
                    setCanSizeInput(String(canSize));
              ...
              <View>
                <View style={[styles.rowBetween, styles.settingInputRow]}>
                  <Text style={styles.subtext}>Price per can (MXN):</Text>

                  <TextInput
                    keyboardType="decimal-pad"
                    value={canPriceInput}
                    onChangeText={setCanPriceInput}
                ...
                <TouchableOpacity
                  ...
                  onPress={async () => {
                    ...
                    await Promise.all([saveCanPrice(canPriceInput), saveCanSize(canSizeInput)]);
          ...
          <View style={[styles.card, styles.cardWhite]}>
            {!showTimerEdit ? (
              <View style={styles.rowBetween}>
                ...
                {timerStart ? (
                  <Text style={styles.subtextSmall}>locked while active</Text>
                ) : (
                  <TouchableOpacity
                    accessibilityLabel="Edit timer threshold"
                    onPress={() => setShowTimerEdit(true)}
                  >
                    <Text style={styles.linkText}>tap to edit</Text>
                  </TouchableOpacity>
                )}
